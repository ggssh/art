...
std::unique_ptr<RuntimeParser> ParsedOptions::MakeParser(bool ignore_unrecognized) {
  ...
  parser_builder->
      ...
      .Define("-XX:Use32GBHeapShiftCompression:_")
          .WithHelp("Enable 32GB heap shift compression for pointer compression")
          .WithType<bool>()
          .WithValueMap({{"false", false}, {"true", true}})
          .IntoKey(M::Use32GBHeapShiftCompression)
      .Define("-XX:YYZDebug:_")
          .WithHelp("Enable YYZ debug")
          .WithType<bool>()
          .WithValueMap({{"false", false}, {"true", true}})
          .IntoKey(M::YYZDebug)
